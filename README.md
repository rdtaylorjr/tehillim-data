...
`benchmark={parallelism,genre,trajectory}/family={lexical,semantic}/stage=W/...`.

* `benchmark=parallelism/family={lexical,semantic}/`: `stage=raw` (retrieval-pair AP/AUC/
...
* `benchmark=genre/family={lexical,semantic}/`: `stage=raw` (genre-discrimination AP/AUC/
  ...
  CSV).
* `benchmark=trajectory/family={lexical,semantic}/`: `stage=profiles` (per-model, per-psalm
  ...
  control.
* `ui_lexical.json`, `ui_semantic.json`: the two family payloads `tehillim-evaluate`'s
  `ui_export.export` produces, spliced into the results UI.
